...
const loadOrderList = async (isRefresh = false) => {
	...
	try {
		...
		// 基础查询条件
		let whereCondition = {
			status: dbCmd.neq('deleted') // 排除已删除的订单
		}
		...
		if (selectedCustomerId.value) {
			whereCondition.customer_id = selectedCustomerId.value
		}

		// 销售类型筛选
		if (filterSaleType.value) {
			whereCondition.sale_type = filterSaleType.value
		}
...
const loadStatistics = async () => {
	try {
		...
		totalSalesFen.value = data.reduce((sum, item) => sum + (item.total_amount_fen || 0), 0)
		totalProfitFen.value = data.reduce((sum, item) => sum + (item.total_profit_fen || 0), 0)
		totalDebtFen.value = data.reduce((sum, item) => sum + (item.debt_amount_fen || 0), 0)
...
const cancelOrder = async () => {
	...
	try {
		...
		const res = await uniCloud.callFunction({
			...
			data: {
				order_id: currentOrder.value._id
			}
		})
		...
		if (res.result.code === 0) {
			...
			showError(res.result.message || '作废失败')
		}
	} catch (error) {
		hideLoading()
