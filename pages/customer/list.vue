	...
	try {
		...
		if (searchKeyword.value) {
			whereCondition = dbCmd.or([
				{ name: new RegExp(searchKeyword.value, 'i') },
				{ phone: new RegExp(searchKeyword.value, 'i') }
			])
		}

		// 客户类型筛选
		if (filterType.value) {
			whereCondition.customer_type = filterType.value
		}

		// 欠款状态筛选
		if (filterDebtStatus.value) {
			switch (filterDebtStatus.value) {
				case 'no_debt':
					// 无欠款：欠款为0
					whereCondition.total_debt_fen = 0
					break
				case 'has_debt':
					// 有欠款：欠款大于0
					whereCondition.total_debt_fen = dbCmd.gt(0)
					break
				case 'over_credit':
					// 超额欠款：欠款超过信用额度（需要用 where 自定义条件）
					// 这里简化处理，后续可以通过云函数实现更复杂的查询
					break
			}
		}
...
const submitForm = async () => {
	...
	try {
		...
		if (res.result.code === 0) {
			...
			showError(res.result.message || '添加失败，请重试')
		}
