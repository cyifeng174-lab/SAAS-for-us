...
const loadInventoryList = async (isRefresh = false) => {
	...
	try {
		...
		if (searchKeyword.value) {
			whereCondition.product_name = new RegExp(searchKeyword.value, 'i')
		}

		// 等级筛选
		if (filterGrade.value) {
			whereCondition.grade = filterGrade.value
		}

		// 规格筛选
		if (filterSpec.value) {
			whereCondition.spec = filterSpec.value
		}
		...
		if (isRefresh) {
			...
			inventoryList.value = [...inventoryList.value, ...(res.data || [])]
		}

		// 更新分页状态
		totalCount.value = inventoryList.value.length
		hasMore.value = (res.data || []).length >= pageSize
		currentPage.value++

		// 提取规格选项（去重）
		extractSpecOptions(res.data || [])
