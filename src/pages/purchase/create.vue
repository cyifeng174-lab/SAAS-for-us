...
import { purchaseAPI, supplierAPI } from '@/utils/api.js'
...
const loadSuppliers = async () => {
	try {
		showLoading('加载供应商...')
		// 调用本地 API：查询激活状态的供应商（最多100条）
		const res = await supplierAPI.list({ status: 'active', page_size: 100 })

		// API 返回 { code, message, data }，data 中 list 为数组
		if (res.code === 0) {
			supplierList.value = res.data.list || []
		} else {
			showError(res.message || '加载供应商失败')
		}
		hideLoading()
	} catch (error) {
		hideLoading()
...
const submitPurchase = async () => {
	...
	try {
		...
		// 调用本地 API 创建采购单
		// 注意：API 返回 { code, message, data } 格式
		const res = await purchaseAPI.create(requestData)

		hideLoading()

		if (res.code === 0) {
			// 提交成功
			// API 返回的 data 可能使用 camelCase，此处做字段名适配
			const apiData = res.data
			successInfo.value = {
				batch_no: apiData.batchNo || apiData.batch_no || '',
				supplier_name: apiData.supplierName || apiData.supplier_name || '',
				fruit_name: apiData.fruitName || apiData.fruit_name || '',
				weight_jin: apiData.weightJin || apiData.weight_jin || 0,
