			...
			<view class="popup-content">
				...
				<scroll-view class="inventory-list" scroll-y>
					<view
						...
						:key="inventory._id"
						:class="{ 'no-stock': inventory.current_stock_jin <= 0 }"
					...
					>
						<view class="inventory-main">
							...
							<text class="no-stock-tag" v-if="inventory.current_stock_jin <= 0">无库存</text>
						</view>
						<view class="inventory-stock">
							<text class="stock-label">库存：</text>
							<text class="stock-value" :class="{ 'low-stock': inventory.current_stock_jin <= 0 || inventory.current_stock_jin <= inventory.warning_stock_jin }">
...
import { salesAPI, customerAPI, inventoryAPI } from '@/utils/api.js'
...
const loadInventory = async () => {
	try {
		...
		// 调用本地 API 获取库存列表
		// 注意：inventoryAPI.list 返回的 data.list 中每个元素包含 FIFO 成本信息
		...
		if (res.code === 0) {
			...
			showError(res.message || '加载库存列表失败')
		}
...
const submitOrder = async () => {
	...
	try {
		...
		if (res.code === 0) {
			// 开单成功
			showSuccess('开单成功')
			// 适配 API 返回数据（可能为 camelCase，做字段名兼容）
