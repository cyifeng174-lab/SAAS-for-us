...
import { financeAPI, customerAPI } from '@/utils/api.js'
...
const loadCustomers = async () => {
	try {
		...
		console.log('收款-客户列表API返回:', res)

		if (res.code === 0) {
			customerList.value = res.data.list || []
		} else {
			showError(res.message || '加载客户列表失败')
		}
...
const loadUnpaidOrders = async (customerId) => {
	try {
		...
		// TODO: 当前 salesAPI 没有按付款状态筛选未结清订单的端点
		// 暂时保留 uniCloud 直接查询逻辑，待后端提供后替换：
		// const res = await salesAPI.history({ customer_id: customerId, payment_status: 'unpaid,partial', page_size: 50 })
...
const submitReceive = async () => {
	...
	try {
		...
		// 调用本地 API 进行收款
		const res = await financeAPI.receive({
		...
		if (res.code === 0) {
			...
			successInfo.value = {
				receive_no: apiData.receiveNo || apiData.receive_no || '',
				amount_fen: apiData.amountFen || apiData.amount_fen || receiveAmountFen.value,
				payment_method: apiData.paymentMethod || apiData.payment_method || selectedMethod.value,
				receive_time: apiData.receiveTime || apiData.receive_time || receiveTime.value,
				customer_name: selectedCustomer.value.name,
				write_off_count: apiData.writeOffCount || apiData.write_off_count || writeOffOrders.value.length,
				remaining_debt_fen: apiData.remainingDebtFen || apiData.remaining_debt_fen || 0
			}
			showSuccessPage.value = true
		} else {
			showError(res.message || '收款失败')
		}
	} catch (error) {
		hideLoading()
